...
const RealTsuggestion = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center h-screen z-1" style={{ fontFamily: 'DM Sans', fontSize: '18px' }}>
      <div className="h-[60%] w-[70%] mx-auto bg-[rgba(0,0,0,0.7)] rounded-2xl">
        <div ref={containerRef} className="p-4 overflow-auto h-full">
          {suggestions.length > 0
            ? suggestions.map((s, index) => <div key={index}>{s.split('\n').map((line, i) => <div key={i}>{line}</div>)}</div>)
